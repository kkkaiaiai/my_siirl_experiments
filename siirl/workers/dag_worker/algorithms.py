...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, weight_factor_in_cpgd="STD_weight", **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.CPGD_PASSK:
        cpgd_calculation_mask = data.batch["response_mask"]
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.CPGD_PASSK_COMBINATION:
        cpgd_calculation_mask = data.batch["response_mask"]
        advantages, returns = core_algos.compute_cpgd_passk_combination_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=cpgd_calculation_mask,
            index=data.non_tensor_batch["uid"],
        )
