...
def compute_gae_advantage_return(
    ...
    with torch.no_grad():
        ...
        advantages = F.masked_whiten(advantages, response_mask)
...
def compute_reinforce_plus_plus_baseline_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, epsilon: float = 1e-6):
    ...
    with torch.no_grad():
        ...
        scores = F.masked_whiten(scores, response_mask) * response_mask
...
def compute_reinforce_plus_plus_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor, gamma: torch.Tensor):
    ...
    with torch.no_grad():
        ...
        advantages = F.masked_whiten(returns, response_mask)
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
        loss = F.masked_mean(loss_mat, loss_mask)
...
def compute_policy_loss(
    ...
    ppo_kl = F.masked_mean(-negative_approx_kl, response_mask)
    ...
    if use_cpgd_loss:
        ...
        pg_clipfrac_lower = F.masked_mean((ratio > clip_ratio_c) * (advantages < 0).float(), response_mask).detach()
    ...
    pg_clipfrac = F.masked_mean(torch.gt(pg_losses2, pg_losses1).float(), response_mask)

    pg_losses3 = -advantages * clip_ratio_c
    clip_pg_losses2 = torch.min(pg_losses3, clip_pg_losses1)
    pg_clipfrac_lower = F.masked_mean(torch.gt(clip_pg_losses1, pg_losses3) * (advantages < 0).float(), response_mask)
...
def compute_entropy_loss(logits, response_mask, loss_agg_mode: str = "token-mean"):
    ...
    """
    # compute entropy
    token_entropy = F.entropy_from_logits(logits)  # (bs, response_len)
...
def compute_value_loss(vpreds: torch.Tensor, returns: torch.Tensor, values: torch.Tensor, response_mask: torch.Tensor, cliprange_value: float, loss_agg_mode: str = "token-mean"):
    ...
    """
    vpredclipped = F.clip_by_value(vpreds, values - cliprange_value, values + cliprange_value)
